    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(toudi !=null){
            ...
            BeanUtils.copyProperties( toudi , view );//把实体数据重构到view中
                ...
                //级联表
                YonghuEntity yonghu = yonghuService.selectById(toudi.getYonghuId());
                if(yonghu != null){
    ...
    */
    ...
    public R save(@RequestBody ToudiEntity toudi, HttpServletRequest request){
        ...
        if(false)
            ...
            toudi.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ToudiEntity> queryWrapper = new EntityWrapper<ToudiEntity>()
            .eq("yonghu_id", toudi.getYonghuId())
            .eq("gongshi_name", toudi.getGongshiName())
            .eq("gongshi_address", toudi.getGongshiAddress())
            ;
...
//                            toudiEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            toudiEntity.setGongshiName(data.get(0));                    //公司名称 要改的
//                            toudiEntity.setGongshiAddress(data.get(0));                    //公司地址 要改的
//                            toudiEntity.setToudiTime(sdf.parse(data.get(0)));          //就业时间 要改的
