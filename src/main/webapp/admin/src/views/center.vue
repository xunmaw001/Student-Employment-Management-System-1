  ...
  <div>
    ...
    >  
     <el-row>
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='gongsi'"  label='所在行业' prop="hanyeTypes">
                 <el-select v-model="ruleForm.hanyeTypes"  placeholder='请选择所在行业'>
                     <el-option
                             v-for="(item,index) in hanyeTypesOptions"
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='gongsi'" label='营业执照展示' prop="gongsiPhoto">
                 <file-upload
                         tip="点击上传照片"
                         action="file/upload"
                         :limit="3"
                         :multiple="true"
                         :fileUrls="ruleForm.gongsiPhoto?ruleForm.gongsiPhoto:''"
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='yonghu'"  label='就业状态' prop="jiuyeTypes">
                 <el-select v-model="ruleForm.jiuyeTypes"  placeholder='请选择就业状态'>
                     <el-option
                             v-for="(item,index) in jiuyeTypesOptions"
                             v-bind:key="item.codeIndex"
                             :label="item.indexName"
                             :value="item.codeIndex">
                     </el-option>
...
// 注册表 公司 的级联表
   ...
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=hanye_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.hanyeTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiuye_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.jiuyeTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    onUpdateHandler() {
                         ...
                         if((!this.ruleForm.hanyeTypes)&& 'gongsi'==this.flag){
                             this.$message.error('所在行业不能为空');
                             return
                         }
                         ...
                         if((!this.ruleForm.jiuyeTypes)&& 'yonghu'==this.flag){
